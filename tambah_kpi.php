...
// Ambil daftar pegawai dari database untuk pilihan dropdown
$query_pegawai = "SELECT kode_pegawai, nama FROM employe ORDER BY nama ASC";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama) || empty($periode)) {
        ...
        $query = "INSERT INTO kpi (kode_pegawai, nama, periode, target_kpi, capaian_kpi, skor, status) 
                  VALUES ('$kode_pegawai', '$nama', '$periode', '$target_kpi', '$capaian_kpi', '$skor', '$status')";
        
        if (mysqli_query($conn, $query)) {
            $success = 'Data KPI berhasil ditambahkan!';
            header("refresh:2;url=kpi.php");
        } else {
            $error = 'Gagal menambahkan data KPI: ' . mysqli_error($conn);
        }
    }
}
?>
    ...
    <div class="dashboard-shell">
        ...
        <main class="main-panel">
            <div class="page-header">
                <h1>Tambah Data KPI</h1>
                <p>Masukkan capaian penilaian kinerja pegawai</p>
            </div>
                ...
                <form method="POST">
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" id="kode_pegawai" name="kode_pegawai" placeholder="Cth: EMP001">
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-grid full">
                        <div class="form-group">
                            ...
                            <input type="text" name="periode" required placeholder="Cth: 2026 - Q1 atau Januari 2026">
                        </div>
                    </div>

                    <div class="form-grid">
                        <div class="form-group">
                            <label>Target KPI (Poin/Skor)</label>
                            <input type="number" step="0.01" name="target_kpi" value="100" required>
                        </div>
                        <div class="form-group">
                            <label>Capaian Realisasi (Poin/Skor)</label>
                            <input type="number" step="0.01" name="capaian_kpi" placeholder="Cth: 90" required>
                        </div>
                    </div>

                    <div class="form-actions">
                        <button type="submit" class="btn-submit">✅ Simpan Data KPI</button>
